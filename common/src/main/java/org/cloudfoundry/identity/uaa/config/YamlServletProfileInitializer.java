...
/**
 * An {@link ApplicationContextInitializer} for a web application to enable it to
 * externalize the environment and logging configuration. A YAML config file is loaded if
 * present and inserted into the environment. In addition if the YAML contains some
 * special properties, some initialization is carried out:
 * 
 * <ul>
 * <li><code>spring_profiles</code> - then the active profiles are set</li>
 * <li><code>logging.config</code> - then log4j is initialized from that location (if it
 * exists)</li>
 * </ul>
 * 
 * @author Dave Syer
 * 
 */
public class YamlServletProfileInitializer implements
	...
	public static final String[] DEFAULT_PROFILE_CONFIG_FILE_LOCATIONS = new String[] {
		"${APPLICATION_CONFIG_URL}", "file:${APPLICATION_CONFIG_FILE}" };
	...
	public void initialize(ConfigurableWebApplicationContext applicationContext) {
		...
		try {
			servletContext.log("Loading YAML environment properties from location: "
					+ resource);
			...
			NestedMapPropertySource properties = new NestedMapPropertySource(
					"servletConfigYaml", map);
			...
			servletContext.log(
					"Error loading YAML environment properties from location: "
							+ resource, e);
		}
	...
	private void applyLog4jConfiguration(ConfigurableEnvironment environment,
		...
		try {
			servletContext.log("Loading log4j config from location: "
					+ log4jConfigLocation);
			Log4jConfigurer.initLogging(log4jConfigLocation);
		} catch (FileNotFoundException e) {
			servletContext.log("Error loading log4j config from location: "
					+ log4jConfigLocation, e);
		}
	...
	private void applySpringProfiles(ConfigurableEnvironment environment,
			ServletContext servletContext) {
